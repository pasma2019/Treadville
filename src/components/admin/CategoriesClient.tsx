  ...
  const handleDelete = (id: string, name: string) => {
    ...
    startTransition(async () => {
      try {
        ...
        showMessage("error", "Could not delete category.");
      }
    });
  };
  ...
  const handleToggle = (id: string, name: string, currentActive: boolean) => {
    ...
    startTransition(async () => {
      try {
        ...
        showMessage("success", `Category ${action}ed.`);
      } catch {
        showMessage("error", "Could not update category.");
      }
    });
  };
      ...
      {categories.length === 0 ? (
        <div className="rounded border-2 border-dashed border-[var(--line-on-light)] py-20 text-center">
          ...
          <p className="mt-2 font-mono text-sm text-[var(--ink-faint)]">
            Add your first category to begin the catalogue.
          </p>
        </div>
      ) : (
