...
function generateEssayNumber(index: number): string {
...
export default function JournalPreview({
  ...
  const essays = articles.slice(0, 3).map((article, index) => ({
    no: generateEssayNumber(index),
    title: article.title,
    excerpt: article.excerpt ?? "",
    meta: article.published_at ? formatArticleDate(article.published_at) : "Field notes",
        ...
        <div className="relative z-10 mx-auto max-w-[var(--content-wide)]">
          ...
          <div className="mt-12 grid grid-cols-1 gap-6 md:mt-16 md:grid-cols-3 md:gap-8">
            <div className="md:col-span-3">
              <Reveal as="div" delay={0} className="journal-card border border-[var(--line-on-light)] bg-[var(--warm-white)] p-10 md:p-12 text-center">
                ...
                <p className="mt-3 text-[0.9375rem] leading-relaxed text-[var(--ink-soft)] max-w-md mx-auto">
                  We're curating field notes on terroir, sourcing, and the people behind Treadville's agricultural products. Check back soon.
                </p>
              </Reveal>
            </div>
          </div>
          ...
          {essays.map((e, i) => (
            ...
            >
              <Reveal
                ...
                className="journal-card group relative overflow-hidden border border-[var(--line-on-light)] bg-[var(--warm-white)] transition-colors hover:border-[var(--accent-sage)]/50"
              >
                {e.image ? (
