      ...
      <div className="relative z-10 mx-auto max-w-[var(--content-wide)]">
        ...
        {essays.length > 0 ? (
          <div className="mt-12 grid grid-cols-1 gap-6 md:mt-16 md:grid-cols-3 md:gap-8">
            {essays.map((e, i) => {
              const card = (
                <article className="journal-card group relative overflow-hidden border border-[var(--line-on-light)] bg-[var(--warm-white)]">
                  {e.image ? (
              ...
              if (e.slug) {
                return (
                  ...
                  >
                    <Link href={`/journal/${e.slug}`} className="block h-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-sage)] focus-visible:ring-offset-2 focus-visible:ring-offset-transparent">
                      {card}
                    </Link>
                  </Reveal>
                );
          ...
          <Reveal as="div" delay={1} className="mt-12 md:mt-16">
            <div className="max-w-2xl border-l-2 border-[var(--gold)] pl-6">
              <p className="font-mono text-[12px] uppercase eyebrow-gold">
                Coming soon
              </p>
              <h3 className="mt-4 font-display text-2xl italic leading-tight text-[var(--ink)] md:text-3xl">
                The first essays
              </h3>
              <p className="mt-3 text-[0.9375rem] leading-relaxed text-[var(--ink-soft)]">
